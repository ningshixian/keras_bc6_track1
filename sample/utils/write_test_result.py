...
def getCSVData(csv_path):
    ...
    print('F480: {}'.format(entity2id_2.get('f4 80')))  # ['Uniprot:Q61549']    ['NCBI gene:13733', 'Uniprot:Q61549']
    return entity2id_2
...
def searchEntityId(s, predLabels, entity_tag_consisteny, lx_tag_consisteny, entity2id, protein2id, gene2id, text_byte):
    ...
    最后是模糊匹配
    '''
    # entity_list, position_list, leixing_list = entity_tag_consisteny, [], lx_tag_consisteny
    # assert len(entity_list) == len(leixing_list)
...
def entity_disambiguation_cnn(entity, entity_id, cnn, x_sen, pos_sen, x_id_dict, position, stop_word):
    type_id = ''
...
def writeOutputToFile(path, predLabels):
    ...
    # 加载实体消歧模型
    cnn = load_model('/home/administrator/PycharmProjects/keras_bc6_track1/sample/ned/data/weights2.02-0.79.hdf5')
        ...
        if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
            ...
            for doc in documents:
                ...
                for passage in passages:
                    ...
                    # lx_list = pmc_id2lx_list[pmc_id]
                    # assert len(entity_list)==len(lx_list)
                    # for i in range(len(entity_list)):
                    #     entity = entity_list[i]
                    #     lx = lx_list[i]
                    #     entity1 = entity
                    ...
                    for i in range(len(entity_list)):
                        ...
                        if len(entity_id) > 1:
                            ...
                            # type_id = entity_disambiguation_cnn(entity, entity_id, cnn, test_x[idx_line], test_pos[idx_line], x_id_dict, position, stop_word)
    ...
    print('exit:{}, not_exit:{}'.format(exit, not_exit))  # exit:5837, not_exit:2735
    print('测试集预测结果写入成功！')
    print('{}个词未找到对应的ID'.format(num_entity_no_id))  # 58
    print('{}个词有歧义'.format(len(words_with_multiId)))  # 1756
    print('完结撒花')

    with open('/home/administrator/PycharmProjects/keras_bc6_track1/sample/ned/data/x_id_dict.pkl', 'wb') as f:
