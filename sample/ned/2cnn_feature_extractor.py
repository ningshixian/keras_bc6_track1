...
def strippingAlgorithm(entity):
    ...
    for com in common:
        if entity_variants1.startswith(com) or entity_variants1.endswith(com):
            entity_variants1 = entity_variants1.replace(com, '')
            entity_variants2 = entity_variants2.replace(com, '')
...
def searchEntityId(entity, entity2id):
    ...
    if entity_variants1 in entity2id:
        ...
        while len(id_list)<num_candidates:
            ...
            if isinstance(train_id_list[k], list) and train_id_list[k][0]:
                id_list.append(train_id_list[k][0])
        else:
            return id_list
    ...
    if entity_variants1 in gene2id:
        ...
        if not Ids or len(Ids)<num_candidates:
            Ids2 = search_id_from_NCBI(query_list)
            if Ids:
                id_list.extend(Ids2)
                entity2id[entity_variants1].extend(Ids2)

        # for query in [entity, entity_variants1, entity_variants2, entity_variants3]:
        #     res_reviewed = u.search(query + '+reviewed:yes', frmt="tab", columns="id", limit=num_candidates)
        #     if isinstance(res_reviewed, int):
        #         print('请求无效\n')
        #     elif res_reviewed:  # 若是有返回结果
        #         print('{} in Uniprot-reviewed'.format(entity))
        #         Ids = extract_id_from_res(res_reviewed)
        ...
        #         break
        #
        # if entity_variants1 not in entity2id:
        #     # Uniprot 数据库API查询-unreviewed
        #     for query in [entity, entity_variants1, entity_variants2, entity_variants3]:
        #         unres_reviewed = u.search(query, frmt="tab", columns="id", limit=num_candidates)
        #         if isinstance(unres_reviewed, int):
        #             print('请求无效\n')
        #         elif unres_reviewed:  # 若是有返回结果
        #             print('{} in Uniprot-unreviewed'.format(entity))
        ...
        #         continue
        #     if record["IdList"]:
        #         print('{} in NCBI-gene'.format(entity))
        #         x = ['NCBI gene:' + record["IdList"][i] for i in range(len(record["IdList"][:num_candidates]))]
        #         id_list.extend(x)
        #         entity2id[entity_variants1] = x
        #         return id_list

    if entity not in id_list:
        print('未找到{}的ID，空'.format(entity))  # 152次出现

    # print(list(set(id_list[:num_candidates])))
    return list(set(id_list[:num_candidates]))
...
def get_s_features(entity, goldenID, zhixin, entity2id, method='S-product'):
    ...
    #         entity = entity.replace(char + ' ', char)
    #         entity = entity.replace(' ' + char, char)
    entity = entityNormalize(entity, None, None)
    ...
    for candidta in candidtaIds:
        ...
        if candidta in Id2synvec:
            ...
            print('未找到{}对应同义词集向量，随机初始化-候选'.format(candidta))
            syn_vec = np.random.uniform(-0.1, 0.1, 200)
...
def get_x_y(entity, id, zhixin, x_sen, pos_sen, position, res):
    '''
    获取实体的上下文及其对应的pos标记
    获取标签
    '''
    x_left, x_pos_left, x_right, x_pos_right, x_id, y = res
    s_feas, x_id_one, labels = get_s_features(entity, id, zhixin, entity2id)
            ...
            if label == 0 or label == 1 or label == 3:
                if entity:
                    ...
                    if IDs[k].startswith('gene:') or IDs[k].startswith('protein:'):
                        ...
                        get_x_y(entity, IDs[k], zhixin, x_sen, pos_sen, position, res)
                    else:
                        print('IDs error!')
                    entity = ''
                    entityId = ''
...
if __name__ == '__main__':
    ...
    # 保存数据
    with open('data/train_cnn.pkl', "wb") as f:
    ...
    # 第二次抽特征时, 保存数据
    with open('data/train_cnn2.pkl', "wb") as f:
